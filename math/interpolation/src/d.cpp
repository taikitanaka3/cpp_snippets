...
bool LinearInterpolate::interpolate(const std::vector<double> &base_index,
  ...
  if (base_index.size() == 0 || base_value.size() == 0) {
    printf("[interpolate] some vector size is zero: base_index.size() = %lu, "
           "base_value.size() = %lu",
  ...
  if (!isIncrease(base_index) || return_index < base_index.front() ||
      base_index.back() < return_index ||
      base_index.size() != base_value.size()) {
  ...
  if (dist_to_forward < 0.0 || dist_to_backward < 0.0) {
    std::cerr
        << "?? something wrong. skip this return_index. base_index[i - 1] = "
  ...
  const double value =
      (dist_to_backward * base_value[i] + dist_to_forward * base_value[i - 1]) /
  ...
  for (int i = -4; i < 4; i++) {
    for (int j = -4; j < 4; j++) {
      ...
      for (std::vector<double> steer_angle_vels : eps_map_) {
        ...
        linear_interp.interpolate(vel_index_, steer_angle_vels, current,
                                  steer_angle_vel_interp);
      ...
      linear_interp.interpolate(steer_angle_vels_interp, voltage_index_, target,
                                voltage);
      std::cout << "(target_vel,current_vel)\t"
                << "(" << target << "," << current << ")"
                << "\t\t"
