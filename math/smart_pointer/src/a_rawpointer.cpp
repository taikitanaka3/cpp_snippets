...
public:
  RawPointer(int val) : raw_ptr_(new int(val)) {}
  ~RawPointer() { delete raw_ptr_; }
  void printVal() { print(*raw_ptr_); }
};
...
void simple_pointer() {
  ...
  rep(i, 10) {
    *raw_ptr += i;
    print("current i=" << *raw_ptr);
  }
  print("raw_ptr=" << *raw_ptr);
  delete raw_ptr; //これを忘れると、メモリリーク
}

int main() {
  simple_pointer();
  RawPointer rawPointer(999);
  rawPointer.printVal();
  return 0;
}
