...
class AutoPointer{
private:
	std::auto_ptr<int> auto_ptr_;
public:
	AutoPointer(int val):auto_ptr_(new int(val)){}
	void printValue()const{print(*auto_ptr_);}
};

void simple_auto_pointer(){
	auto_ptr<int> auto_ptr(new int(10));
	rep(i,10){
		*auto_ptr+=i;
		print("current i="<<*auto_ptr);
	}
	print("auto_ptr="<<*auto_ptr);
}

int main(){
	simple_auto_pointer();
	AutoPointer autoPointer(999);
	autoPointer.printValue();
	return 0;
}
