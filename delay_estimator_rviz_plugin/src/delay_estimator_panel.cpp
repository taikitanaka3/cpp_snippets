...
: rviz_common::Panel(parent)
{
  ...
  {
    estimate_delay_button_ptr_ = new QPushButton("Estimate Delay");
    connect(estimate_delay_button_ptr_, &QPushButton::clicked, this, &DelayEstimatorPanel::onClickEstimation);
    reset_button_ptr_ = new QPushButton("Reset Data");
    connect(reset_button_ptr_, &QPushButton::clicked, this, &DelayEstimatorPanel::onClickResetData);
    h_layout->addWidget(estimate_delay_button_ptr_);
    h_layout->addWidget(reset_button_ptr_);
    ...
    connect(duration_, SIGNAL(valueChanged(int)), this, SLOT(onDurationChanged));
...
void DelayEstimatorPanel::onInitialize()
{
  ...
  sub_input_ = raw_node_->create_subscription<InputSubscriberType>(
    "/input", 1, std::bind(&DelayEstimatorPanel::onInputTopic, this, _1));
  sub_response_ = raw_node_->create_subscription<ResponseSubscriberType>(
    "/response", 1, std::bind(&DelayEstimatorPanel::onResponseTopic, this, _1));
}

void DelayEstimatorPanel::onDurationChanged(){
    std::cerr<<"change duration: "<<duration_->value()<<std::endl;
}
void DelayEstimatorPanel::onClickEstimation(){
    timer_->start(100);
}
...
void DelayEstimatorPanel::save(rviz_common::Config config) const { Panel::save(config); }

void DelayEstimatorPanel::load(const rviz_common::Config & config) { Panel::load(config); }
