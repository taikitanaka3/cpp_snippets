...
TEST(boost_test, intersects)
{
  ...
  //    +/  |
  //        |
  boost::geometry::read_wkt("polygon((-10 -10, 10 -10, 10 10, -10 10))", occupancy_poly);
  std::cout << boost::geometry::wkt(occupancy_poly) << std::endl;

  Point2d ego_pos = {0, 0};
  Point2d obj_min_theta_pos = {2, 2};
  Point2d obj_max_theta_pos = {0, 2};
  const double obj_min_theta = std::atan2(obj_min_theta_pos.y() - ego_pos.y(), obj_min_theta_pos.x() - ego_pos.x());
  const double obj_max_theta = std::atan2(obj_max_theta_pos.y() - ego_pos.y(), obj_max_theta_pos.x() - ego_pos.x());
  const double ray_max_length = 50.0;
  LineString2d theta_min_ray = {ego_pos, {ray_max_length * std::cos(obj_min_theta), ray_max_length * std::sin(obj_min_theta)}};
  LineString2d theta_max_ray = {ego_pos, {ray_max_length * std::cos(obj_max_theta), ray_max_length * std::sin(obj_max_theta)}};
  ...
  occlusion_poly.outer() = {obj_min_theta_pos,min_intersections.at(0),max_intersections.at(0),obj_max_theta_pos};
  std::cout << boost::geometry::wkt(occlusion_poly) << std::endl;
  occlusion_poly.outer() = {min_intersections.at(0),obj_min_theta_pos,max_intersections.at(0),obj_max_theta_pos};
  ...
  std::cout << boost::geometry::wkt(hull) << std::endl;
  {
    ...
    polygon occ;
    bg::exterior_ring(occ) = boost::assign::list_of<point>
      (1,2)
      (2,3);
