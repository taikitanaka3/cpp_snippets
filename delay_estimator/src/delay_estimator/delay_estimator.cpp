...
EstimationResult DelayEstimator::estimate(
  ...
  smoothed_input_ = lowpassFilter(input, smoothed_input_ ,params.filter.cutoff_hz_in, params.data.sampling_hz);  
  smoothed_response_ = lowpassFilter(response, smoothed_response_ ,params.filter.cutoff_hz_in, params.data.sampling_hz);  
  ...
  if(update_result_ = UpdateResult::QUEQUE){
    ...
    const size_t valid_max_delay_index = static_cast<size_t>(pd.total_data*pt.valid_delay_index_ratio);
    ...
    const double fitlered_value = lowpassFilter(delay_time, staticstic_.p_value ,params.filter.cutoff_hz_out, params.data.sampling_hz);
...
: Node("delay_estimator", node_options){
  ...
  auto &pt = params_.thresh;
  {
    ...
    pt.validation_data_stddev = declare_parameter("thresh/validation_data_stddev", 0.2);
    pt.valid_peak_cross_correlation = declare_parameter("thresh/valid_peak_cross_correlation", 0.8);
    pt.valid_delay_index_ratio =declare_parameter("thresh/valid_delay_index_ratio", 0.1);
  }
  ...
  {
    pf.use_lowpass_filter = declare_parameter("filter/use_lowpass_filter", true);
