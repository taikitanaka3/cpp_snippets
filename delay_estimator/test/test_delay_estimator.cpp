...
TEST(test, vector_cmp) {
  ...
  const size_t max_index = getPeakCrossCorrelationCoefficientIndex(
      input, response, weight, cross_correlation, 3);
...
TEST(test, process_data) {
  ...
  for (size_t i = 0; i < in.size(); i++) {
    const UpdateResult update =
        updateData(params, in.at(i), re.at(i), input_data, response_data);
