...
{
  const double average = std::accumulate((arr).begin(), (arr).end(), 0.0) / static_cast<double>(arr.size());
  double diff = 0;
...
template<class T>
double getCovarianceFromVector(const T & x, const T & y, const double avg_x, const double avg_y, const T & w)
{
  double cov = 0;
  double sum_w = 0;
 ...
 */
 template<class T>
inline size_t getPeakCrossCorrelationCoefficientIndex(
  const T & input, const T & response, const T & weight, T & cross_correlation,
  const size_t num_estimation)
{
  ...
  for (size_t tau = 0; tau < num_estimation; tau++) {
    const size_t peak_index = calcCrossCorrelationCoefficient(
        ...
        {response.rbegin(), response.rend() - tau},
        weight);
    cross_correlation.emplace_back(peak_index);
  }
  const auto & iter = std::max_element(cross_correlation.begin(), cross_correlation.end());
...
inline UpdateResult updateData(
