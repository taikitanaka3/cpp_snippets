...
TEST(same_case, spline) {
  ...
  std::vector<double> out = ::interpolation::slerp(base_keys,base_values,query_keys);
  for (auto o:out)
  {
    std::cout<<o<<std::endl;
  }  
...
TEST(almost_same_point, spline_linear_cmp) {
  ...
  std::vector<double> s_query_vals = ::interpolation::slerp(base_keys,base_vals,query_keys);
  std::vector<double> l_query_vals = ::interpolation::lerp(base_keys,base_vals,query_keys);
  for (size_t i = 0; i < query_keys.size(); i++)
  {
    std::cout<<"s key : "<<query_keys.at(i)<<" s val : "<<s_query_vals.at(i)<<std::endl;
    std::cout<<"l key : "<<query_keys.at(i)<<" l val : "<<l_query_vals.at(i)<<std::endl;
  }
